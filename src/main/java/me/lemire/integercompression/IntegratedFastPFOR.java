...
public final class IntegratedFastPFOR implements IntegratedIntegerCODEC {
        ...
        private void encodePage(int[] constin, IntWrapper constinpos,
                int thissize, int[] out, IntWrapper outpos,
                IntWrapper initoffset) {
                ...
                for (final int finalconstinpos = tmpconstinpos + thissize
                        - BLOCK_SIZE; tmpconstinpos <= finalconstinpos; tmpconstinpos += BLOCK_SIZE) {
                        ...
                        for (int k = 0; k < 128; k += 32) {
                                BitPacking.fastpack(buffer, k, out, tmpoutpos,
                                        tmpbestb);
                                tmpoutpos += tmpbestb;
                        }
                }
        ...
        private void decodePage(int[] in, IntWrapper inpos, int[] out,
                ...
                for (int run = 0, run_end = thissize / BLOCK_SIZE; run < run_end; ++run, tmpoutpos += BLOCK_SIZE) {
                        ...
                        initoffset.set(Delta.fastinverseDelta(out, tmpoutpos,
                                BLOCK_SIZE, initoffset.intValue()));
                }
                outpos.set(tmpoutpos);
                inpos.set(inexcept);
        }
